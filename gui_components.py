...
class PortSymbol:
    ...
    def on_drag(self, event):
        if self.dragging and self.canvas.data["active_line"] is not None:
            ...
            control_x1 = start_x + (end_x - start_x) / 2
            control_y1 = start_y
            control_x2 = start_x + (end_x - start_x) / 2
            control_y2 = end_y
            self.canvas.coords(
    ...
    def update_wire(self, line, source_port, target_port):
        if DEBUG:
            print(f"Connecting {source_port.port['name']} to {target_port.port['name']}")
        self.canvas.coords(line, 
                           source_port.x, source_port.y,
                           (source_port.x + target_port.x) / 2, source_port.y,
                           (source_port.x + target_port.x) / 2, target_port.y,
                           target_port.x, target_port.y)
    ...
    def create_adapter_connections(self, source_port, adapter_left_port, target_port, adapter_right_port):
        line1 = self.canvas.create_line(
            source_port.x, source_port.y,
            adapter_left_port.x, adapter_left_port.y,
            fill=source_port.color if source_port.color else "black",
            tags=("wire",),
            smooth=True,
            splinesteps=36,
            width=3
        ...
        line2 = self.canvas.create_line(
            adapter_right_port.x, adapter_right_port.y,
            target_port.x, target_port.y,
            fill=source_port.color if source_port.color else "black",
            tags=("wire",),
            smooth=True,
...
class AdapterBlock:
    def __init__(self, canvas, x, y, metaA, metaB, mode, inherited_color=None):
        ...
        self.left_port = PortSymbol(
            self.canvas, self.x - 30, self.y,
            self, {"name": "Din", "dir": "in", "type": ist},
            inside=True,
            inherited_color=inherited_color
        )
        self.right_port = PortSymbol(
            self.canvas, self.x + 30, self.y,
            self, {"name": "Dout", "dir": "out", "type": ost},
            inside=True
        )
    ...
    def remove_all_connections(self):
        ...
        for port in [self.left_port, self.right_port]:
            connections = [conn for conn in self.canvas.data["connections"] if conn[0] == port or conn[1] == port]
