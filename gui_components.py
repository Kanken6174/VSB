...
class PortSymbol:
    def __init__(self, canvas, x, y, block, port):
        ...
        self.dragging = False
        self.is_conduit = False  # Initialize conduit flag
    ...
    def export_as_conduit(self):
        if self.is_conduit:
            return  # Already a conduit

        # Change port color to red
    ...
    def remove_conduit(self):
        if not self.is_conduit:
            return  # Not a conduit

        # Change port color back to default (e.g., black)
...
class EntityBlock:
    def __init__(self, canvas, x, y, name, generics, ports, conduit=False):
        self.canvas = canvas
        self.name = name
        self.generics = generics  # List of generics
        self.ports = ports
            ...
            try:
                for gen in self.generics:
                    ...
                    if gen['type'].lower() == 'integer':
                        ...
                        # Ensure string is enclosed in quotes
